...
class VictimAgent():
    ...
    def __init__(self, env, MEMORY_SIZE, discount_factor=1.0, alpha=0.1, epsilon=0.1):
        ...
        self.Q = defaultdict(lambda: self.np.zeros(self.env.action_space.n))
...
#                     print(f"Timsteps = {t} | Scores = {score}")
    ...
    def Show_PolicyQ(self):
        policy_Q = self.np.zeros(self.env.shape)

        for i in self.Q:
            best_action = self.np.argmax(self.Q[i])
            policy_Q[self.np.unravel_index(i, self.env.shape)] = best_action
    ...
    def Get_Policy(self):
        policy = self.np.zeros((self.env.nS, self.env.nA))
        for i in self.Q:
            best_action = self.np.argmax(self.Q[i])
