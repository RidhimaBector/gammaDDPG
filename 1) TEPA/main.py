...
if __name__ == "__main__":
    ...
    for i_episode in range(args.max_episodes_num):
        ...
        for t in range(args.max_timesteps):
            ...
            # Step: cost
            distance = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, system.victim.Q, TARGET, cost_matrix, distance_type=0) #system.victim.Q
            distance_grid = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, system.victim.Q, TARGET, cost_matrix, distance_type=1)
            distance_behavior = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, system.victim.Q, TARGET, cost_matrix, distance_type=2)
            done, accuracy, accuracy_sftmx, accuracy_sftmx_complete = utils_attack.Attack_Done_Identify(env, TARGET, system.victim.Q) #system.victim.Q)
        ...
        if no_episodes % args.eval_freq_episode == 0:
            ...
            system.ae.save(f"./{model_dir}/{no_episodes}")
