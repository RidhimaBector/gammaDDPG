...
class ReplayBuffer(object):
	def __init__(self, state_dim, action_dim, max_size=int(1e6)):
		...
		self.distance = np.zeros((max_size, 1))
		self.not_done = np.zeros((max_size, 1))

		self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")


	def add(self, state, action, next_state, reward, distance, done):
		self.state[self.ptr] = state
		self.action[self.ptr] = action
		self.next_state[self.ptr] = next_state
		self.reward[self.ptr] = reward
		self.distance[self.ptr] = distance
	...
	def saveBuffer(self, filename):
		...
		np.save(filename +'_distance.npy', self.distance)
	...
	def loadBuffer(self, filename):
		...
		self.distance = np.load(filename +'_distance.npy')
