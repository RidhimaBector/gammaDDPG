...
class DDPG(object):
    ...
    def modify_discount(self, distance_batch):
        distance_batch = distance_batch.clip(5.0, 40.0)
        denominator = np.max(distance_batch) - np.min(distance_batch)
        modified_discount = np.where( (denominator!=0), (0.19 * ((distance_batch - np.min(distance_batch)) / denominator )) + 0.8, 0.9)
        return modified_discount
    ...
    def train(self, replay_buffer, atk_n_epoch, atk_n_batch, batch_size, ddpg_loss, i_episode):
        ...
        for i_atk_n_epoch in range(atk_n_epoch):
            ...
            for i_atk_n_batch in range(atk_n_batch):
                ...
                reward_batch, distance_batch, terminal_batch = replay_buffer.sample(self.batch_size) #Algorithm 2 Line 14
                ...
                modified_discount = self.modify_discount(distance_batch)
                
                target_q_batch = to_tensor(reward_batch) + \
                    to_tensor(modified_discount)*to_tensor(terminal_batch.astype(np.float))*next_q_values #Algorithm 2 Line 15
