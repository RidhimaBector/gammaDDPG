...
if __name__ == "__main__":
    ...
    for i_episode in range(args.max_episodes_num):
        ...
        for t in range(args.max_timesteps):
            ...
            # Step: cost
            distance = - utils_attack.Attack_Cost_Compute_W(env, INIT_T, victim.Q, TARGET, cost_matrix, distance_type=0) 
            distance_grid = - utils_attack.Attack_Cost_Compute_W(env, INIT_T, victim.Q, TARGET, cost_matrix, distance_type=1) #Dynamic Discount TargetWD - Algorithm 2 Line 12
            distance_behavior = - utils_attack.Attack_Cost_Compute_W(env, INIT_T, victim.Q, TARGET, cost_matrix, distance_type=2)
            ...
            # Replay buffer
            Buffer.add(x.view(state_dim), u, next_x.view(state_dim), reward, -distance_grid, done) #$\gamma$DDPG with Dynamic Discount (-discount) TargetWD - Algorithm 2 Line 13
